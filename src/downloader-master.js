...
const { graphNoLayoutTemporary, infoName } = require('./config')
const { deleteFolder, ensureDirExists } = require('./utils')
const { saveInfo } = require('./files')
const logger = require('./log')

const { generate } = require('./generator-master')
...
module.exports = (start, end) => {
    ...
    chunkNumber = Math.ceil((lastBlock - firstBlock) / chunkSize)
    ...
    function startWorkers(infuraApiKey) {
        ...
        for (var i = 0; i < CPUs; i++) {
            var child = child_process.fork('./build/downloader-worker')
...
const progressBarMsg = `Retrieving transactions...`
var chunkNumber
var firstBlock
var lastBlock
var nextBlock 
...
module.exports = (start, end) => {
    ...
    chunkNumber = lastBlock - firstBlock + 1
    const progressBar = logger.progress(progressBarMsg, chunkNumber)
